...
\begin{definition} \index{Metrik}
	Sei $M$ eine nichtleere Menge. Eine Abbildung $d: M \times M \rightarrow \MdR$ hei{\ss}t \textbf{Metrik} auf $M$, falls $\forall x, y, z \in M:$
		\begin{enumerate}
			\item $d(x, y) \geq 0, \hspace{0.25cm} d(x, y) = 0 \gdw x = y $  (positive Definitheit)
...
\begin{beispiel}
	\begin{enumerate}[label=\alph*\upshape)]	
		\item Sei $X$ ein Vektorraum und $p_{j}$ für $j \in \MdN$ Halbnormen auf $X$ mit der Eigenschaft, dass für jedes $x \in X \ {0}$ ein $K \in \MdN$ exisitert mit $p_{K} > 0$. Dann definieren wir
	\end{enumerate}
