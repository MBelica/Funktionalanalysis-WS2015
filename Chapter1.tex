...
\begin{beispiel}
$X = Y = l^{p}, e_{n} = (\delta_{n})_{m \in \MdN}$	\\
$T e_{j} = \sum_{i \in \MdN} a_{ij} e_{i} \text{ mit } a_{ij} \in \MdR: \left( \sum_{i = 1}^{\infty} |a_{ij}|^p \right)^{\frac{1}{p}} < \infty$ \\ \\
Dann gilt $X = \sum_{j = 1}^{\infty} x_{j} e_{j}, \space{0.25cm}$ mit $x_{j} \in \MdR$, $\space{0.25cm} \| x \| = \left( \sum |x_{i}|^{p} \right)^{\frac{1}{p}}$ \\
$\hspace{1cm} [ Tx ]_{i} = \sum_{j = 1}^{\infty} a_{ij} x_{j} $ (Matrizenmultiplikation) \\
$T \in B(l^{p}) \rightarrow A = (a_{ij})_{i,j \in \MdN}$
\end{beispiel}
