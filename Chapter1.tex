...
\begin{prop}
Für zwei Normen $\| . \|_{1}, \| . \|_{2}$ auf $X$ sind äquivalent:
\begin{enumerate}
	\item $\| . \|_{1}, \| . \|_{2}$ sind äquivalent
	...
	\item Es gibt Konstanten $0 < m$, $M < \infty$, so dass $m U_{(X, \| . \|_{1})} \leq U_{(X, \| . \|_{2})} \leq M U_{(X, \| . \|_{1})}$
\end{enumerate}
\end{prop}
\begin{beweis}
	todo	
...
\begin{beispiel}
	...
	\end{itemize}
Gültigkeit der Dreiecksungleichung beweist man ähnlich wie bei $(\MdK^{n}, \| . \|_{p})$.
\end{beispiel}
...
\begin{bemerkung}
Im unendlich dimensionalen Fall sind die Normen $\| . \|_{p}$ auf $\MdF$ nicht äquivalent.	
\end{bemerkung}
\begin{beweis}
todo	
\end{beweis}
...
\begin{bemerkung} \\
Auf $C_{b}^{m}[0, 1]$ ist eine äquivalente Norm zu  $\| f \|_{C_{b}^{m}}$ gegeben durch\begin{align*} \|f\|_{0} & = \sum_{i = 0}^{m - 1} |f^{(i)}(0)| + \| f^{(m)} \|_{\infty} \\
