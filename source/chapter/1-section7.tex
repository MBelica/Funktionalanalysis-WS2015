...
\begin{beispiel} \label{bsp:7.2}
	...
	\[ \gdw \sup_{\|x\|_{p} \leq 1} \left( \sum_{j = n + 1}^{\infty} \left| (Tx)_{j} \right|^{p} \right)^{\frac{1}{p}} \xrightarrow[n \rightarrow \infty]{} 0 \gdw T(U_{X}) \text{ ist relativ kompakt in } \ell^{p} \text{ nach } \hyperref[bsp:6.4]{6.4} \]
	
	\begin{enumerate}[label=\alph*\upshape)]
		\item $T(x_{j}) \coloneqq (\lambda_{j} x_{j})_{j \in \MdN}$ mit $\lambda_{j} \in \MdK$ Diagonaloperator \\
			...
			\begin{beweis}
				...
				Sei umgekehrt: $\lambda_{j} \not\rightarrow 0, \text{ dann } \exists \lambda_{j_{k}} \rightarrow \lambda \neq 0 $
				\[ T(e_{i_{k}}) = \lambda_{i_{k}} e_{i_{k}}, ~ T(\lambda_{j_{k}}) \approx \lambda e_{j_{k}} \text{ hat keine konvergente Teilfolge.} \]
			\end{beweis}
		\item $T(x_{i}) \coloneqq (0, x_{1}, x_{2}, x_{3}, \dotsc)$ $"$Shift$"$ \\
			Isometrie! $\| T(x_{j}) \| = \| x_{i} \| \Rightarrow$ nicht kompakt.
		\item $\left[ T(x_{j}) \right]_{i} \coloneqq \sum_{j = 1}^{\infty} a_{ij} x_{j}$. Dann ist $T \in B(\ell^{p})$, falls $\left( \sum_{i} \left( \sum_{j} | a_{ij} |^{q} \right)^{\frac{p}{q}} \right)^{\frac{1}{p}} < \infty$, $\frac{1}{p} + \frac{1}{p^{'}} = 1. $\\
			Diese Hille-Tamerkin-Operatoren sind kompakt.
			\begin{beweis}
				\[ \| \left[ Q_{n} T(x_{j}) \right]_{i} \| \leq \left( \sum_{j = n + 1}^{\infty} \left( \sum_{j} |a_{ij}|^{q} \right)^{\frac{p}{q}} \right)^{\frac{1}{p}} \rightarrow 0, n \rightarrow \infty \]
			\end{beweis}
	\end{enumerate}
\end{beispiel}
...
\begin{beweis}
	\begin{enumerate}[label=\alph*\upshape)]
		...
		\item Sei $\| x_{n} \|_{X} \leq 1$ und $S$ kompakt $\Rightarrow \exists n_{k}: S( T x_{n_{k}} )$ ist eine Cauchy-Folge, denn $T x_{n_{k}}$ ist beschränkt. \\
			Sei $\| x_{n} \|_{X} \leq 1$ und $T$ kompakt $\Rightarrow \exists n_{k}: T x_{n_{k}}$ ist eine Cauchy-Folge, damit ist aber auch $S(T x_{n_{k}})$ eine Cauchy-Folge, da $S$ stetig ist.
	\end{enumerate}	
\end{beweis}
