...
\begin{definition}
	...
	\begin{enumerate}[label=\alph*\upshape)]
		...
		\item $T$ hei{\ss}t \textbf{Isomorphismus}, \index{Isomorphismuss} falls $T$ bijektiv und stetig ist und $T^{-1}: Y \rightarrow X$ ebenfalls stetig ist. 
			...
			In diesem Fall Identifizieren wir $X \cong Y$ und sagen $X$ und $Y$ sind isomorph. (da $X, Y$ normierte Vektorräume sind, fordern wir im Gegensatz zur Linearen Algebra, dass $T, T^{-1}$ zusätzlich stetig)
	\end{enumerate}
\end{definition}
...
\begin{beispiel}
	...
	\begin{beweis}
		...
		\begin{align*}
			...
			& \leq \| y' \|_{(\ell^{p})'} \| \underbrace{ \sum_{n = 1}^{N} z_{n} e_{n} }_{= \left( \sum_{n = 1}^{N} |x_{n}|^{(q - 1)p} \right)^{\frac{1}{p}}} \|_{\ell^{p}} \\
		...
		\begin{align*}
			...
			& = \sum_{n = 1}^{\infty} y'(y_{n} e_{n}) \hspace{0.25cm} \forall y \in \ell^{p} \\
			& = y'(y)			
		\end{align*}
...
\begin{bemerkung*}
	\begin{enumerate}[label=\alph*\upshape)]
		...
		\item Eine ähnliche Aussage gilt auch für $L^{p}$-Räumen auf einem Ma{\ss}raum $(\Omega, \mathcal{A}, \mu):$
			Hier gilt:
			\[ L^{p}(\Omega, \mu)' \cong L^{q}(\Omega, \mu) \]
			wobei $p \in [1, \infty), \hspace{0.25cm} \frac{1}{p} + \frac{1}{q} = 1$ bezüglich der Dualität: $[\Phi_{p}(f)](g) (= \langle f, g \rangle ) = \int_{\Omega} f(x) g(x) d\mu(x)$ 
	\end{enumerate}	
\end{bemerkung*}
