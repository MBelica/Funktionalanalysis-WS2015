...
\begin{beweis}
	...
	$" \leq "$: $M = \sup_{\| x \| \leq 1} \< T x , x \>$ \\
	Zu zeigen ist $\| T \| \leq M$: \\
	Es gilt $T^{*} = T$ und nach der Definition von $M$
	\begin{align*}
...
\begin{beweis}
	\begin{enumerate}[label=\alph*\upshape)]
		...
		\item $T x = \lambda x, x \neq 0: \lambda \< x, x \> = \< T x , x \> = \< x , T x \> = \overline{\lambda} \< x , x \>$ \\
			...
			\end{align*} 
			Da $y = \lim_{n} \left( \lambda_{n} x_{n} \right)$: $T y = \lambda \lim T x_{n} = \lambda y$. \\
			$\Rightarrow \lambda \in \sigma(T), \lambda \in \MdR, | \lambda | = \| T \|$.
	\end{enumerate}	
\end{beweis}
...
\begin{bemerkung*}
	Falls $X$ seperabel ist, so gibt es eine Orthonormalbasis $(e_{n})$ von $X$, die diese $(h_{n})$ und eine orthonormalbasis von $\kernn(T)$ so, dass 
	\[ T x = \sum_{n = 0}^{\infty} \mu_{n} \< x , e_{n} \> e_{n} \]
	wobei $\mu_{n} = 0$, falls $e_{n} \in \kernn(T), \mu_{n} = \lambda_{m}$ falls $e_{n} = h_{m}$.
\end{bemerkung*}

There was a drawing here % todo at 19.11: add drawing
...
\end{align*}
Da für jedes $k$ $(e_{1}^{k}, \dotsc, e_{d_{k}}^{k})$ ein Orthonormalsystem von $X_{k}$ ist und $X_{k} \bot X_{l}$ für $k \neq l$ nach \hyperref[]{9.11b}, ist $(h_{n})$ ein Orthonormalsystem in X. \\ % todo: is it 19.10b? prof weis wrote 9.11b...
Also: $T h_{n} = \lambda_{n} h_{n}$ und $h_{n} \bot \kernn(T)$ nach \hyperref[]{9.11b} \\ % todo: is it 19.10b? prof weis wrote 9.11b... 
...
Beweis: für $x \in X_{0}^{\bot}$ ist $Tx \in X_{0}^{\bot}$, denn $\< T x , h_{n} \> = \< x , T h_{n} \> = \lambda \underbrace{\< x , h_{n} \>}_{= 0}, \quad \forall n$ \\
