...
\begin{beispiel}
	\begin{enumerate}[label=\alph*\upshape)]
		...
		\item \index{diskrete Metrix} \label{bsp:1-diskreteMetrik} Sei $M$ eine nichtleere Menge, dann definieren wir die \textbf{diskrete Metrik} auf $M$ durch
...
\begin{definition} \index{abgeschlossen} \index{offen} \label{def:1-4.4-abgoff}
...
\begin{bemerkung}
	\begin{enumerate}[label=\alph*\upshape)]
		...
		\index{relative Abgeschlossenheit}
...
\begin{beispiel} \label{bsp:1-4.11}
	\begin{enumerate}[label=\alph*\upshape)]
		...
		\item Sei $X$ ein normierter Vektorraum und $Y \subseteq X$ abzählbar mit $\overline{lin Y} = X$. Dann ist $X$ separabel.
			\begin{beweis}
				Wir definieren % todo: what is that ... + i ...
		...
		\item Der Raum $\ell^{\infty}$ ist nicht separabel: \\
			...
			% todo: ist it really 1/4 as radius for the balls (three times)
