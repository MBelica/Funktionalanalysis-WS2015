...
\begin{beispiel*}
	$ \Omega = \MdR, \ca_{m} = \{ [\frac{n - 1}{2^{m}}, \frac{n}{2^{m}}) : n = -2^{2m}, \dotsc, 0, \dotsc, 2^{2m} \}$, dann ist die Feinheit $r_{m} = 2^{m}$.
\end{beispiel*}
...
\begin{beweis}[siehe \ref{satz:7.7}]
	...
	Mit \hyperref[satz:8.4]{Satz 8.4} bzw. $S_{n} = \EW_{\ca_{n}}$ folgt die Behauptung.
\end{beweis}
