\subsection*{2 - Normierte Räume}
	...
	\begin{karte}{Halbnorm}
		Falls $ \| \cdot \| $ all die Eigenschaften einer Norm erfüllt au{\ss}er $ \| x \| = 0 \Rightarrow x = 0 $, dann hei{\ss}t $ \| \cdot \| $ \begriff{Halbnorm}.
	\end{karte}
	...
	\begin{karte}{Folgenraum}	
		$\MdF = \{ (x_{n}) \in \mathbb{K}^{\mathbb{N}}: x_{i} = 0 \text{ bis auf endlich viele } n \in \mathbb{N} \} $ ist der \begriff{Folgenraum} und $e_{j} = (0, \dotsc, 0, 1, 0, \dotsc, 0) $ der j-te Einheitsvektor in $\MdF$, wobei die $1$ an j-ter Stelle steht.
	\end{karte}		
